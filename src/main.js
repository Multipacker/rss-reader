...
const updateFeeds = async () => {
    ...
    const newFeeds = rawFeeds
        .filter(({ url, text, }) => text.length !== 0)
        .map(({ url, text, }) => {
            let xmlFeed = {};
            try {
                xmlFeed = xmlParser.parse(text, true);
            } catch (error) {
                console.error(`Failed to parse XML of '${url}': ${error.message}`);
            }
            ...
            if (xmlFeed.rss) {
                ...
                if (rss.title) {
                    feed.title = rss.title["#text"];
                }

                if (rss.description) {
                    feed.description = rss.description["#text"];
                }

                if (rss.link && rss.link["#text"]) {
                    feed.link = feed.id = rss.link["#text"];
                } else {
                ...
                if (rss.lastBuildDate) {
                    feed.updated = rss.lastBuildDate["#text"];
                } else if (rss.pubDate) {
                    feed.updated = rss.pubDate["#text"];
                }
            } else if (xmlFeed.feed) {
                const atom = xmlFeed.feed;

                if (atom.title) {
                    feed.title = atom.title["#text"];
                }

                // NOTE(simon): At most one
                if (atom.subtitle) {
                    feed.description = atom.subtitle["#text"] ?? "";
                }

                // NOTE(simon): Should
                if (atom.link) {
                    feed.link = atom.link.find(link => link["@_rel"] === "self")["@_href"] ?? url;
                } else {
                    feed.link = url;
                }
                ...
                feed.articles = atom.entry.map(entry => {
                    ...
                    if (Array.isArray(entry.link)) {
                        ...
                        if (link_node) {
                            article.link = link_node["@_href"] ?? link_node["#text"];
                        }
                    }
